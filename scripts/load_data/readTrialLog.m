...
%
    ...
    while(~feof(fid))
        ...
        if(strcmp(splits{2}, 'PropDriftController') && strcmp(splits{3}, 'Entering state Measured'))
            trial.time_drift = datenum8601(splits{1});
            trials(end + 1) = trial;
        end
        
        % When trial is finished, add it to the list of trials
        if(strcmp(splits{2}, 'ExperimentController') && strcmp(splits{3}, 'Event ProprioceptiveDrift Measured'))
